...
(define (install-dense-termlist-package)
  ...
  (define (tag term-list)
    (attach-tag 'dense-termlist term-list))
  ; Public API
  (put 'adjoin-term-fn '(dense-termlist)
       (lambda (term-list)
         (lambda (term)
...
(define (install-sparse-termlist-package)
  ...
  (define (tag term-list)
    (attach-tag 'sparse-termlist term-list))
  ; Public API
  ; cheat and return a term-adding fn so the type-list is nicer
  (put 'adjoin-term-fn '(sparse-termlist)
       (lambda (term-list)
         (lambda (term)
           (tag (adjoin-term term term-list)))))
...
(define (coeff term) (cadr term))
(define (adjoin-term term term-list)
  ; Cheat here by having 'adjoin-term-fn return a lambda that adds terms
  ((apply-generic 'adjoin-term-fn term-list) term))
(define (the-empty-termlist)
